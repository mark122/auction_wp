            ...
            <div class="col">
				...
				if ( ! empty( $_POST['action'] ) && ! empty( $_POST['wp_support_field'] ) ) {
					...
					if ( wp_verify_nonce( $_POST['wp_support_field'], 'wp_support_action' )
						...
						if ( empty( $name ) ) {
							$error[] = esc_attr__( 'Please, Enter your Name.', 'popup-box' );
						}
						if ( empty( $email ) ) {
							$error[] = esc_attr__( 'Please, Enter your Email.', 'popup-box' );
						}
						if ( empty( $subject ) ) {
							$error[] = esc_attr__( 'Please, Enter Subject of Message.', 'popup-box' );
						}
						if ( empty( $message ) ) {
							$error[] = esc_attr__( 'Please, Enter your Message.', 'popup-box' );
						}
						...
						if ( count( $error ) == 0 ) {
							...
							echo '<div class="notice notice-success is-dismissible"><p>'
							     . esc_attr__( 'Your Message sent to the Support.', 'popup-box' ) . '</p></div>';
						}
					} else {
                            ...
                            <div class="field has-addons">
                                <div class="control">
									...
									<select name="type" class="is-radiusless">
										<option value="Issue"><?php esc_attr_e( 'Issue', 'popup-box' ); ?></option>
										<option value="Idea"><?php esc_attr_e( 'Idea', 'popup-box' ); ?></option>
									</select>
									</span>
                                </div>
