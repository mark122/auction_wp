...
$this->wpaws_wpconfig('add','AWS_CDN_Domain',$this->wpadbval['id']);
	...
	$dls = $result['DistributionList']['Items'];
		...
		{
		echo "<div class='alert alert-info'>No distribution Found on Amazon Cloudfront for " . esc_html($this->wpadbval['domain']) . ".</div>";
		wp_die();
		}
	...
	public function wpaac_delete_certificate()
	{
		...
		echo "<div class='gscols12 alert alert-info'>SSL Certificate has been deleted</div>";
		}
	wp_die();
	}
	...
	public function wpaac_list_certificate()
	{
		...
		echo "<div class='gscols12 alert alert-info'><button alt='Delete Certificate' title='Delete Certificate' data-id=".esc_html($certificate['CertificateArn'])." class='gsformbtnmini alert-danger wpawsssldelete'><span class='dashicons dashicons-trash'></span></button> &nbsp; &nbsp; Inactive certificate found for <strong>".esc_html($certificate['DomainName'])."</strong>. Please verify & click <strong>List Certificate</strong> again</div>";
		wp_die();
		}
		}
		
		if($found == false) echo "<div class='alert alert-info'>No SSL certificate found for " . esc_html($this->wpawscdndomain) ." </div>";
...
/* add to wp-config*/
	...
	try
	{
		...
		echo "<div class='alert alert-success gscols12'>SSL certificate has been issued and is curently pending verification.</div>";
			...
			echo  "<div class='alert alert-success gscols12'>An email has been sent to one of these addresses:<ul>"; 
				...
				echo "</ul><br>Please check & approve the email from Amazon.<p><div class='alert alert-info'>Once the certificate is verified, return to this page and click <strong>List Certificate</strong>.</div></p><button id=wpawscdn_resend class='gsformbtn hidden'>Re-send Validation Email</button></div>";
			}
		}
	...
	public function wpaws_sanitize($REQUEST,$check = NULL)
	{
		...
		{
		echo "<div class='alert alert-info'>Please provide a valid Access ID / Secret Key</div>";
		wp_die();
		}
