...
class WC_Product_Auction extends WC_Product {
	...
	public function get_price_html( $price = '' ) {
		...
		if ($this->is_closed() && $this->is_started() ){
			...
			$price = '<span class="auction-price starting-bid" data-auction-id="'.$id.'" data-bid="'.$this->get_auction_current_bid().'" data-status="future">'.__('<span class="starting auction">Reserve:</span> ','wc_simple_auctions').wc_price($this->get_curent_bid()).'</span>';

		} else {

			if ( $this->get_auction_sealed() == 'yes' ){
				$price = '<span class="auction-price" data-auction-id="'.$id.'"  data-status="running">'.__('<span class="current auction aa">This is sealed bid auction.</span> ','wc_simple_auctions').'</span>';

			} else {
				if ( !$this->get_auction_current_bid() ) {
					$price = '<span class="auction-price starting-bid" data-auction-id="'.$id.'" data-bid="'.$this->get_auction_current_bid().'" data-status="running">'.__('<span class="current auction">Reserve:</span> ','wc_simple_auctions').wc_price($this->get_curent_bid()).'</span>';
				} else {
