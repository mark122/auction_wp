...
class WonderPlugin_Carousel_View {
	...
	function add_metaboxes() {
		add_meta_box('overview_features', __('Wonder Carousel Features', 'wonderplugin_carousel'), array($this, 'show_features'), 'wonderplugin_carousel_overview', 'features', '');
	...
	function print_overview() {
		...
		<h2><?php echo __( 'Wonder Carousel', 'wonderplugin_carousel' ) . ( (WONDERPLUGIN_CAROUSEL_VERSION_TYPE == "C") ? " Pro" : ( (WONDERPLUGIN_CAROUSEL_VERSION_TYPE == "L") ? " Lite" : " Trial")) . " Version " . WONDERPLUGIN_CAROUSEL_VERSION; ?> </h2>
	...
	function print_edit_settings() {
		...
		<tr valign="top">
			<th scope="row">When adding images from Media Library, use the following text field to populate the title of the carousel image</th>
			<td>
				<select name="titlefield">
				  <option value="title" <?php echo ($titlefield == 'title') ? 'selected="selected"' : ''; ?>>Title</option>
				  <option value="caption" <?php echo ($titlefield == 'caption') ? 'selected="selected"' : ''; ?>>Caption</option>
				</select>
			</td>
		</tr>
	...
	function print_register() {
		...
		function validateLicenseForm() {
			...
			if (jQuery.trim(jQuery("#wonderplugin-carousel-key").val()).length <= 0)
			{
	...
	function print_add_new() {
		...
		$settings = $this->controller->get_settings();
		echo $this->creator->render( -1, null, $settings['thumbnailsize'], $settings['titlefield'] );
	}
	...
	function print_edit_item()
	{
		...
		$this->creator = new WonderPlugin_Carousel_Creator($this);
		$settings = $this->controller->get_settings();
		echo $this->creator->render( $_REQUEST['itemid'], $this->controller->get_item_data( $_REQUEST['itemid'] ), $settings['thumbnailsize'], $settings['titlefield'] );
	}
	...
	?>
		...
		<h2><?php _e( 'Tools', 'wonderplugin_carousel' ); ?></h2>
		...
		<ul class="wonderplugin-tab-buttons-horizontal" id="wp-tools-toolbar" data-panelsid="wonderplugin-popup-display-panels">
			...
			<li class="wonderplugin-tab-button-horizontal"><span class="dashicons dashicons-desktop" style="margin-right:8px;"></span><?php _e( 'PHP Info', 'wonderplugin_carousel' ); ?></li>
		</ul>
