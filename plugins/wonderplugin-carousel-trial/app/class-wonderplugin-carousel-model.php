...
class WonderPlugin_Carousel_Model {
	...
	function generate_body_code($id, $itemname, $has_wrapper, $inline_content, $attributes) {
		...
		if ($item_row != null)
		{
			...
			if (isset($data->customcss) && strlen($data->customcss) > 0)
			{
				...
				if ($removeinlinecss)
				{
					$cssjs .= 'wondercarousel_' . $id . '_appendcss("' . $this->escape_css_for_js($customcss) . '");';
				}
				else
				{
			...
			if (isset($data->showimagehoverovereffect) && strtolower($data->showimagehoverovereffect) === 'true' && !empty($data->imagehoverovercss))
			{
				$imgcss = str_replace(array("\r", "\n", "CAROUSELID"), array(" ", " ", $id), $data->imagehoverovercss);
				if ($removeinlinecss)
				{
					$cssjs .= 'wondercarousel_' . $id . '_appendcss("' . $this->escape_css_for_js($imgcss) . '");';
				}
				else
				{
					$ret .= '<style>' . $imgcss . '</style>';
				}
			}
			...
			if (isset($data->skincss) && strlen($data->skincss) > 0)
			{
				...
				if ($removeinlinecss)
				{
					$cssjs .= 'wondercarousel_' . $id . '_appendcss("' . $this->escape_css_for_js($skincss) . '");';
				}
				else
				{
			...
			if ($has_wrapper)
			{
				$containerstyle .= 'max-width:' . ( ( isset($data->fullwidth) && (strtolower($data->fullwidth) === 'true') ) ? '100%;' : ($data->width * $data->visibleitems . 'px;')) . 'margin:0 auto;';
			}

			if ( isset($data->paddingtop) && isset($data->paddingright) && isset($data->paddingbottom) && isset($data->paddingleft))
			{
				$containerstyle .= 'padding: ' . $data->paddingtop . 'px ' . $data->paddingright . 'px ' . $data->paddingbottom . 'px ' . $data->paddingleft . 'px;';
			}

			if ( !empty( $containerstyle ) )
			{
				$ret .= ' style="' . $containerstyle . '"';
			}
			...
			$boolOptions = array('donotzoomin', 'addpreloading', 'multiplebyrow', 'showimgtitle','sameheight', 'sameheightresponsive', 'fullwidth', 'centerimage', 'fitimage', 'fitcenterimage', 'fixaspectratio', 'autoplay', 'random', 'autoplayvideo', 'circular', 'pauseonmouseover', 'continuous', 'responsive', 'showhoveroverlay', 'showhoveroverlayalways', 'hidehoveroverlayontouch', 'lightboxresponsive', 'lightboxshownavigation', 'lightboxnogroup', 'lightboxshowtitle', 'lightboxshowdescription', 'lightboxaddsocialmedia', 'usescreenquery', 'donotinit', 'addinitscript', 'doshortcodeontext',
					...
					'limitvisibleitemstolength', 'hidearrowsandbulletsforonepage', 'showfirstlasthalf', 'verticalflexheight', 'hidebulletssmallscreen', 'removeinlinecss',
					'lightboxinityoutube', 'lightboxinitvimeo',
			...
			$valOptions = array('preloadingimage', 'spacing', 'rownumber', 'visibleitems', 'arrowstyle', 'arrowimage', 'arrowwidth', 'arrowheight', 'navstyle', 'navimage', 'navwidth', 'navheight', 'navspacing', 'hoveroverlayimage', 'lightboxthumbwidth', 'lightboxthumbheight', 'lightboxthumbtopmargin', 'lightboxthumbbottommargin', 'lightboxbarheight', 'lightboxtitlebottomcss', 'lightboxdescriptionbottomcss', 'continuousduration',
					...
					'firstlasthalfsmallscreen', 'hidebulletssmallscreensize', 'movemode', 'moveduration',
			...
			if (empty($inline_content) && isset($data->slides) && count($data->slides) > 0)
			{
				...
				foreach ($items as $index => $slide)
				{
					...
					else if ($slide->type == 13)
					{
						...
						$ret .= do_shortcode($slide->htmlcode);
						$ret .= '</div>';
					...
					else
					{
						...
						if ($slide->type == 0 && !empty($slide->hoverimage))
						{
							$image_code .= ' data-hoverimage="' . $slide->hoverimage . '"';
						}
			...
			if (!empty($cssjs))
			{
				$ret .= '<script>function wondercarousel_' . $id . '_appendcss(csscode) {var head=document.head || document.getElementsByTagName("head")[0];var style=document.createElement("style");head.appendChild(style);style.type="text/css";if (style.styleSheet){style.styleSheet.cssText=csscode;} else {style.appendChild(document.createTextNode(csscode));}};' . $cssjs . '</script>';
			}
		...
		if ($query->have_posts())
		{
			while ( $query->have_posts() )
			{
				...
				if ($post)
				{
					...
					$postdata['featured_image_size'] = $featured_image_size;
	...
	function create_custom_post_code($slides, $data, $id) {
		...
		foreach($slides as $slide)
		{
			...
			$skin_template = str_replace('__DATETIME__',  isset($slide->datetime) ? $slide->datetime : '', $skin_template);
			$skin_template = str_replace('__DATE__',  isset($slide->date) ? $slide->date : '', $skin_template);
			$skin_template = str_replace('__AUTHOR__',  isset($slide->author) ? $slide->author : '', $skin_template);
		...
		foreach($slides as $slide)
		{
			...
			if ( isset($slide->categories) )
			{
				...
				foreach($slide->categories as $catindex => $cat)
				{
					...
					$cat_names[] = $cat['name'];
					$cat_names_with_links[] = '<a href="' . $cat['link'] . '" ' . (isset($data->categorylinkextraprops) ? $data->categorylinkextraprops : '') . ' >' . $cat['name'] . '</a>';
				}
				...
				$skin_template = str_replace('__CATEGORYNAMES__',  implode(', ', $cat_names), $skin_template);
				$skin_template = str_replace('__CATEGORYNAMESANDLINKS__',  implode(', ', $cat_names_with_links), $skin_template);
			}
			...
			if ( isset($slide->tags) )
			{
				...
				$skin_template = str_replace('__TAGNAMES__',  implode(', ', $tag_names), $skin_template);
				$skin_template = str_replace('__TAGNAMESANDLINKS__',  implode(', ', $tag_names_with_links), $skin_template);
			}
	...
	function get_page_object($options, $post)
	{
		...
		$post_item = array(
				...
				'datetime'		=> $post['post_date'],
				'date'			=> date('Y-m-d', strtotime($post['post_date'])),
				'author'		=> issset($post['post_author']) ? get_the_author_meta( 'display_name', $post['post_author'] ) : "",
		);
	...
	function get_settings() {
		...
		$titlefield = get_option( 'wonderplugin_carousel_titlefield', 'title' );
	...
	function save_settings($options) {
		...
		if (isset($options) && isset($options['titlefield']))
			$titlefield = $options['titlefield'];
		else
			$titlefield = 'title';
		update_option( 'wonderplugin_carousel_titlefield', $titlefield );
