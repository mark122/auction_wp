    ...
    <form id="wcmp-edit-product-form" class="woocommerce form-horizontal" method="post">
            ...
            <div class="right-primary-info"> 
                <div class="form-group-wrapper">
                    <div class="form-group product-short-description">
                        <label class="control-label col-md-12 pt-0" for="product_short_description"><?php esc_html_e( 'Product short description', 'ibid' ); ?></label>
                        <div class="col-md-12">
                            <?php
                            $settings = array(
                            ...
                            wp_editor( wp_specialchars_decode( $product_object->get_short_description( 'edit' ) ), 'product_excerpt', $settings );
                            ?>  
                        </div>
                    </div>
                    
                    <div class="form-group product-description">
                        <label class="control-label col-md-12" for="product_description"><?php esc_attr_e( 'Product description', 'ibid' ); ?></label>
                        <div class="col-md-12">
                            <?php
                            $settings = array(
            ...
            <div class="left-primary-info">
                <div class="product-gallery-wrapper">
                    <div class="featured-img upload_image"><?php $featured_img = $product_object->get_image_id( 'edit' ) ? $product_object->get_image_id( 'edit' ) : ''; ?>
                        <a href="#" class="upload_image_button tips <?php echo esc_attr($featured_img) ? 'remove' : ''; ?>" <?php echo current_user_can( 'upload_files' ) ? '' : 'data-nocaps="true" '; ?>data-title="<?php esc_attr_e( 'Product image', 'ibid' ); ?>" data-button="<?php esc_attr_e( 'Set product image', 'ibid' ); ?>" rel="<?php echo esc_attr( $post->ID ); ?>">
                            <div class="upload-placeholder pos-middle">
                                <i class="wcmp-font ico-image-icon"></i>
                                <p><?php _e( 'Click to upload Image', 'ibid' );?></p>
                            </div>
                            <img src="<?php echo esc_attr($featured_img) ? esc_url( wp_get_attachment_image_src( $featured_img, 'medium' )[0] ) : esc_url( wc_placeholder_img_src() ); ?>" />
                    ...
                    <div id="product_images_container" class="custom-panel">
                        <h3><?php _e( 'Product gallery', 'ibid' );?></h3>
                            ...
                            if ( ! empty( $attachments ) ) {
                                foreach ( $attachments as $attachment_id ) {
                                    ...
                                    echo '<li class="image" data-attachment_id="' . esc_attr( $attachment_id ) . '">
                                            ' . $attachment . '
                                            <ul class="actions">
                                                <li><a href="#" class="delete tips" data-tip="' . esc_attr__( 'Delete image', 'ibid' ) . '">' . __( 'Delete', 'ibid' ) . '</a></li>
                                            </ul>
                                        </li>';
                        ...
                        <input type="hidden" id="product_image_gallery" name="product_image_gallery" value="<?php $product_image_gallery; ?>" />
                        <p class="add_product_images">
                            <a href="#" <?php echo current_user_can( 'upload_files' ) ? '' : 'data-nocaps="true" '; ?>data-choose="<?php esc_attr_e( 'Add images to product gallery', 'ibid' ); ?>" data-update="<?php esc_attr_e( 'Add to gallery', 'ibid' ); ?>" data-delete="<?php esc_attr_e( 'Delete image', 'ibid' ); ?>" data-text="<?php esc_attr_e( 'Delete', 'ibid' ); ?>"><?php _e( 'Add product gallery images', 'ibid' ); ?></a>
                        </p>
                    </div>
                </div>
            </div> 
                ...
                <div id="woocommerce-product-data" class="add-product-info-holder">   
                    ...
                    <div class="add-product-info-header row-padding">
                        <div class="select-group">
                            <label for="product-type"><?php esc_html_e( 'Product Type', 'ibid' ); ?></label>
                            <select class="form-control inline-select" id="product-type" name="product-type">
                                ...
                                <?php endforeach; ?>
                                <option value="auction"><?php echo esc_html( 'Auction','ibid' ); ?></option>
                            </select>
                        </div>
                        <?php
                            ...
                            <ul class="nav nav-tabs" role="tablist" id="product_data_tabs">
                                <?php foreach ( $self->get_product_data_tabs() as $key => $tab ) : ?>
                                    <?php if ( apply_filters( 'wcmp_afm_product_data_tabs_filter', ( ! isset( $tab['p_type'] ) || array_key_exists( $tab['p_type'], wcmp_get_product_types() ) && $WCMp->vendor_caps->vendor_can( $tab['p_type'] ) ), $key, $tab ) ) : ?>
                                        <li role="presentation" class="<?php echo esc_attr( $key ); ?>_options <?php echo esc_attr( $key ); ?>_tab <?php echo esc_attr( isset( $tab['class'] ) ? implode( ' ', (array) $tab['class'] ) : ''  ); ?>">
                                            <a href="#<?php esc_attr_e( $tab['target'],'ibid' ); ?>" aria-controls="<?php echo esc_attr($tab['target']); ?>" role="tab" data-toggle="tab"><span><?php echo esc_html( $tab['label'] ); ?></span></a>
                                        </li>
                                    <?php endif; ?>
                                <?php endforeach; ?>
            ...
            <div class="col-md-8">
                ...
                <div class="panel panel-default pannel-outer-heading order-action">
                    <div class="panel-heading">
                        <?php esc_html_e( 'Rejection Note', 'ibid' ); ?>
                    </div>
                        ...
                        <ul class="order_notes list-group mb-0">
                            <li class="list-group-item list-group-item-action flex-column align-items-start add_note">
                                ...
                                <?php wp_nonce_field('dc-vendor-add-product-comment', 'vendor_add_product_nonce'); ?> 
                                    <h3><?php _e( 'Add note', 'ibid' ); ?> <span class="img_tip" data-desc="<?php echo __( 'Add a note for your reference, or add a customer note (the user will be notified).', 'ibid' ); ?>"></span></h3>
                                    <div class="form-group">
                                        <textarea placeholder="<?php _e('Enter text ...', 'ibid'); ?>" class="form-control" name="product_comment_text"></textarea>
                                    </div>
                                    <div class="form-group">
                                        <input class="btn btn-default wcmp-add-order-note" type="submit" name="wcmp_submit_product_comment" value="<?php _e('Submit', 'ibid'); ?>">
                                    </div>
                                    <input type="hidden" name="product_id" value="<?php echo esc_attr($post->ID); ?>">
                                    <input type="hidden" name="current_user_id" value="<?php echo esc_attr($vendor->id); ?>">
                                <!--  </form>  --> 
                                <?php endif; ?>  
                            </li>
                            <li class="list-group-item list-group-item-action flex-column align-items-start"><div class="form-group"><h3><?php esc_html_e( 'Communication Log', 'ibid' ); ?></h3></div></li>
                            <?php
                            if ($notes) {
                                foreach ($notes as $note) {
                                    ...
                                    <li class="list-group-item list-group-item-action flex-column align-items-start order-notes">
                                        ...
                                        <p><?php echo esc_attr($note->comment_author); ?><?php echo esc_attr($Seller); ?> - <?php echo  date_i18n(wc_date_format() . ' ' . wc_time_format(), strtotime($note->comment_date) ); ?></p>
                                    </li>
                                    <?php
                                }
                            }else{
                                echo '<li class="list-group-item list-group-item-action flex-column align-items-start order-notes">' . __( 'There are no notes yet.', 'ibid' ) . '</li>';
                            }
                            ?>
                        </ul>
                    </div>
            ...
            <div class="col-md-4">
                ...
                <?php if ( $product_categories ) : ?>
                    <div class="panel panel-default pannel-outer-heading">
                        <div class="panel-heading">
                            <h3 class="pull-left"><?php esc_html_e( 'Product categories', 'ibid' ); ?></h3>
                        </div>
                        <div class="panel-body panel-content-padding form-group-wrapper"> 
                            <?php
                            echo esc_attr($product_categories);
                            ?>
                        </div>
                    </div>
                <?php endif;
                ...
                if( $custom_taxonomies ){
                    foreach ( $custom_taxonomies as $taxonomy ) {
                        ...
                        if ( $taxonomy->public && $taxonomy->show_ui && $taxonomy->meta_box_cb ) { ?>
                            <div class="panel panel-default pannel-outer-heading">
                                <div class="panel-heading">
                                    <h3 class="pull-left"><?php $taxonomy->label; ?></h3>
                                </div>
                                <div class="panel-body panel-content-padding form-group-wrapper">
                                    <div class="form-group">
                                        <div class="col-md-12">
        ...
        <?php if ( ! empty( wcmp_get_product_types() ) ) : ?>
            <div class="wcmp-action-container">
                ...
                <input type="submit" class="btn btn-default" name="draft-data" value="<?php esc_attr_e( 'Draft', 'ibid' ); ?>" id="wcmp_afm_product_draft" />
                <input type="hidden" name="status" value="<?php  get_post_status( $post ); ?>">
